        ...
        <main class="contents">
            ...
            <div id="wrap">
                <ul id="gallery">
                    <%
                    	int page_num=0; 
                    	String view_page=request.getParameter("page"); //페이지 파라메터를 가져온다
                    	if(view_page==null){	//파라메터가 없으면 NULL 일테니 page_num을 0으로 설정한다
                    		page_num=0;
                    	}
                    	else{		//파라메터가 있다면 파라메터를 Integer형으로 변환한뒤 page_num에 값을 대입한다
                        	page_num=Integer.parseInt(view_page);
                    	}
                    	
                    	int list_count=0;//사용자 화면에 표시되는 게시글의 개수를 세는 변수
                    	int count2=0;	 //현재 몇번 반복했는지 세는 변수 
                    	if(list!=null){	 //list가 Null이 아닐때만 실행
                    	String s="";
						for(ImageVO vo:list){
							if(vo.getVisit()==1){ //판매완료글이라면 확대하지 않는다.
                        		s = "style='transform:scale(1);' ";
                        	}else{
                        		s = "";
                        	}
							if(count2>=page_num*8 && count2<(page_num+1)*8){
							//현재 페이지에 맞게 page_num을 계산하여 8개만 출력하도록 한다.
					%>
					
							<li class="product_line" id="drop-content">
							
							
		                        <a href="./details/product.jsp?no=<%=vo.getNo()%>&title=<%=vo.getTitle() %>&price=<%=vo.getPrice() %>&s_region=<%=vo.getRegion() %>&content=<%=vo.getContent() %>&
s_userId=<%=vo.getUserId() %>&fileName=<%=vo.getFileName() %>&visit=<%=vo.getVisit() %>&writeDate=<%=vo.getWriteDate() %>&userNickname=<%=vo.getUserNickname()%>">
	...
	s_userId=<%=vo.getUserId() %>&fileName=<%=vo.getFileName() %>&visit=<%=vo.getVisit() %>&writeDate=<%=vo.getWriteDate() %>&userNickname=<%=vo.getUserNickname()%>">
        ...
        <div class="prd_page" >
         	<%if(page_num-1<0){
       		 %>
       		 <div class="prd_box" >
       		 	<a href="./index.jsp?page=<%=page_num%>" class="p_center"><%=page_num+1 %></a> 
         		<a href="./index.jsp?page=<%=page_num+1%>" class="p_left"><%=page_num+2 %></a> 
         		<a href="./index.jsp?page=<%=page_num+2%>" class="p_right"><%=page_num+3 %></a> 
       		 </div>
       		 <%
       		 }
       		 ...
       		 %>
         	<div class="prd_box" >
	         	<a href="./index.jsp?page=<%=page_num-2%>" class="p_left"><%=page_num-1 %></a>  
	         	<a href="./index.jsp?page=<%=page_num-1%>" class="p_right"><%=page_num %></a> 
	         	<a href="./index.jsp?page=<%=page_num%>" class="p_center"><%=page_num+1 %></a> 
        	</div>
			<%
			}
         	...
         	<div class="prd_box" >
         		...
         		<a href="./index.jsp?page=<%=page_num-1%>" class="p_left"><%=page_num %></a>  
	         	...
	         	<a href="./index.jsp?page=<%=page_num+1%>" class="p_right"><%=page_num+2 %></a>
	         	<%} %> 
         	</div>
         	<%
         	}
